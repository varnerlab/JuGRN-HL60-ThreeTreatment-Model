...
for parameter_set_index = starting_set:number_of_initial_sets
...
error_array_true = Float64[]
# through away outliers
for true_error_index = 1:length(error_array)
  if error_array[true_error_index] <= 500 # cut off can be changed
    push!(error_array_true,error_array[true_error_index])
  end
end
variance_error = var(error_array_true);
epsilon = 0.5;
...
for parameter_set_index = 1:number_of_final_sets
  ...
  if (error_value<=error_cutoff)
    ...
    prob_keep_set = exp(-1*error_difference/(2*variance_error));
    check_role = rand();
...
for (local_index,parameter_set_index) in enumerate(ensemble_index_array)
  ...
  # dump data to disk -
  local_data = [T X];
  data_filename = "./simple_ensemble/sim_data_EI_"*string(parameter_set_index)*"_LI_"*string(local_index)*".dat"
