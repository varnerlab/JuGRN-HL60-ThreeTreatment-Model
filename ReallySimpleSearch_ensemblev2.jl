  ...
  while (should_loop_continue == true)
    ...
    if (LAMBDA_SUCC == 1)
      ...
      if error_array[1] <= error_tolerance
        ...
        writedlm("ensemble_may14_archive2.dat",parameter_archive);

        # Save the error_archive -
        error_archive = [error_archive error_array[1,1]];
        writedlm("error_may14_archive2.dat", error_archive);
      end
      ...
      writedlm("parameter_best_may14_v2.dat",parameter_array);
...
for sample_index in collect(2:2)
 ...
 previous_parameter_array = readdlm("parameter_best_may14_v1.dat")
  ...
  for index = 1:length(parameter_mapping_array)
    ...
    # Write parameters to disk ...
    @show sample_index
    writedlm("parameter_best_may14_v1.dat",parameter_array);
