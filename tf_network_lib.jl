...
# EDITS JAN 20
# OBJ function of CD14 have been scaled --- 3x to have similar contributions as CD38 and CD 11b
...
experiment_array_unmeasured = tmp_value["experiment_array_unmeasured"] # unmeasured species used for initial conditions
# preload the data -
...
function objective_function(parameter_array)
  ...
  # indices of species that are not experimentally measured
  ...
  for (experiment_index,experiment_dictionary) in enumerate(experiment_array_RA) # FOR RA
    ...
    #experimental_data_array_RA[experiment_index] = experimental_data_array[end,2]
  ...
  for (experiment_index,experiment_dictionary) in enumerate(experiment_array_unmeasured) # FOR RA
  ...
  OBJ_TOTAL = [obj_array_RA;obj_array_D3;obj_array_RA_D3*0;obj_unmeasured];
